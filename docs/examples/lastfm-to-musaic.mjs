#!/usr/bin/env node
//
// EXAMPLE converter: Last.fm scrobbles -> Musaic listening import file.
//
...
// rather than depending on it. Musaic does not ship converters.
//
//   node docs/examples/lastfm-to-musaic.mjs scrobbles.json musaic-lastfm.json
//
...
function convert(scrobbles) {
  ...
  rows.forEach((row, i) => {
    // Last.fm is in SECONDS, Musaic in MILLISECONDS. Musaic rejects a file that is entirely in
    ...
    // and Musaic would then report more listening time than actually elapsed.
    const next = rows[i + 1]
...
const [inputPath, outputPath = 'musaic-lastfm.json'] = process.argv.slice(2)
if (!inputPath) {
  console.error('usage: node lastfm-to-musaic.mjs <scrobbles.json> [output.json]')
  process.exit(1)
}
